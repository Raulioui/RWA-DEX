 ...
 */
contract dIBTA is FunctionsClient, ConfirmedOwner, ERC20, Pausable {
    ...
    */
    ...
    mapping(bytes32 requestId => AssetRequest request) private s_requestIdToRequest;
    ...
    */
        ...
        returns (bytes32 requestId)
    {
        ...
        s_requestIdToRequest[requestId] = AssetRequest(amountOfTokensToMint, msg.sender, MintOrRedeem.mint);
        return requestId;
    }

    /*
     * @notice user sends a Chainlink Functions request to sell IB01 for redemptionCoin
    ...
    function sendRedeemRequest(uint256 amountdIBTA) external whenNotPaused returns (bytes32 requestId) {
        ...
        uint256 amountIb01InUsdc = getUsdcValueOfUsd(getUsdValueOfIb01(amountdIBTA));
        if (amountIb01InUsdc < MINIMUM_REDEMPTION_COIN_REDEMPTION_AMOUNT) {
        ...
        args[1] = amountIb01InUsdc.toString();
        req._setArgs(args);

        // Send the request and store the request ID
        // We are assuming requestId is unique
        requestId = _sendRequest(req._encodeCBOR(), i_subId, GAS_LIMIT, s_donID);
        s_requestIdToRequest[requestId] = AssetRequest(amountdIBTA, msg.sender, MintOrRedeem.redeem);
    ...
    */
    ...
    function _redeemFulFillRequest(bytes32 requestId, bytes memory response) internal {
        // USDC returned from the brokerage by the selling of IB01
    ...
    */
    ...
    // IB01 USD has 8 decimal places, so we add an additional 10 decimal places
    function getIb01Price() public view returns (uint256) {
    ...
    function getUsdValueOfIb01(uint256 ib01Amount) public view returns (uint256) {
        return (ib01Amount * getIb01Price()) / PRECISION;
    }
    ...
    function getTotalUsdValue() public view returns (uint256) {
        return (totalSupply() * getIb01Price()) / PRECISION;
    }

    function getCalculatedNewTotalValue(uint256 addedNumberOfIb01) public view returns (uint256) {
        return ((totalSupply() + addedNumberOfIb01) * getIb01Price()) / PRECISION;
    }

    function getRequest(bytes32 requestId) public view returns (AssetRequest memory) {
